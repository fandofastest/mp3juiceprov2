...
async function seedDatabase(mongoUri) {
    utils_1.Logger.info("Starting database seeding...");
    const uri = mongoUri || process.env.MONGODB_URI || "mongodb://localhost:27017/mp3juice";
    await (0, index_1.connectToDatabase)(uri);
    // 1. Seed Super Admin
    const superAdminEmail = "superadmin@mp3juice.pro";
    const existingSuper = await index_1.User.findOne({ email: superAdminEmail });
    if (!existingSuper) {
        const pwHash = await (0, auth_1.hashPassword)("admin12345");
        ...
        utils_1.Logger.info("Super Admin user created (superadmin@mp3juice.pro / admin12345).");
    }
    else {
        utils_1.Logger.info("Super Admin already exists.");
    }
    // 2. Seed Settings
    const settingsCount = await index_1.SystemSettings.countDocuments();
    if (settingsCount === 0) {
        await index_1.SystemSettings.create({
        ...
        utils_1.Logger.info("Default system settings seeded.");
    }
    // 3. Seed Categories
    ...
    for (const cat of categoriesList) {
        await index_1.Category.findOneAndUpdate({ slug: cat.slug }, { $setOnInsert: { isDeleted: false }, ...cat }, { upsert: true });
    }
    utils_1.Logger.info("Default categories seeded and synchronized.");
    // 4. Seed Banners
    const bannerCount = await index_1.Banner.countDocuments();
    if (bannerCount === 0) {
        await index_1.Banner.create({
        ...
        utils_1.Logger.info("Default banner seeded.");
    }
    // 5. Seed Home Builder Sections
    const sectionCount = await index_1.HomeSection.countDocuments();
    if (sectionCount === 0) {
        const sections = [
            {
        ...
        await index_1.HomeSection.insertMany(sections);
        utils_1.Logger.info("Default Home Sections seeded.");
    }
    utils_1.Logger.info("Database seeding complete.");
}
// Keep executable support for CLI runner (npm run seed)
if (process.argv[1] && process.argv[1].includes("seed")) {
    seedDatabase().then(() => process.exit(0)).catch((err) => {
        utils_1.Logger.error("Failed to seed database:", err);
        process.exit(1);
    });
}
