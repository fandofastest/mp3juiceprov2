...
exports.default = PlayLogsPage;
const react_1 = require("react");
const api_client_1 = require("../../lib/api-client");
const lucide_react_1 = require("lucide-react");
function PlayLogsPage() {
    ...
    const fetchLogs = (0, react_1.useCallback)(async () => {
        ...
        try {
            ...
            const res = await (0, api_client_1.apiRequest)(`/play-logs?${params.toString()}`);
            setLogs(res.logs || []);
    ...
    return (<div className="space-y-8 max-w-7xl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <button onClick={() => fetchLogs()} disabled={loading} className="flex items-center gap-2 px-4 py-2.5 bg-stone-900 hover:bg-stone-800 border border-stone-800 rounded-lg text-xs font-semibold text-stone-200 transition shrink-0 self-start sm:self-auto">
          <lucide_react_1.RefreshCw className={`w-4 h-4 text-emerald-400 ${loading ? "animate-spin" : ""}`}/>
          Refresh Logs
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="p-6 rounded-xl bg-stone-900 border border-stone-800 flex items-center justify-between">
          ...
          <div className="w-12 h-12 rounded-xl bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center">
            <lucide_react_1.PlayCircle className="w-6 h-6 text-emerald-400"/>
          </div>
        </div>
        ...
        <div className="p-6 rounded-xl bg-stone-900 border border-stone-800 flex items-center justify-between">
          ...
          <div className="w-12 h-12 rounded-xl bg-purple-500/10 border border-purple-500/20 flex items-center justify-center">
            <lucide_react_1.Activity className="w-6 h-6 text-purple-400"/>
          </div>
        </div>
        ...
        <div className="p-6 rounded-xl bg-stone-900 border border-stone-800 flex items-center justify-between">
          ...
          <div className="w-12 h-12 rounded-xl bg-blue-500/10 border border-blue-500/20 flex items-center justify-center">
            <lucide_react_1.Music className="w-6 h-6 text-blue-400"/>
          </div>
        </div>
        ...
        <div className="p-6 rounded-xl bg-stone-900 border border-stone-800 flex items-center justify-between">
          ...
          <div className="w-12 h-12 rounded-xl bg-orange-500/10 border border-orange-500/20 flex items-center justify-center">
            <lucide_react_1.Smartphone className="w-6 h-6 text-orange-400"/>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 rounded-xl bg-stone-900 border border-stone-800 flex flex-col md:flex-row items-center justify-between gap-4">
        <form onSubmit={handleSearchSubmit} className="flex gap-2 w-full md:w-96">
          <div className="relative flex-1">
            <lucide_react_1.Search className="w-4 h-4 text-stone-500 absolute left-3 top-1/2 -translate-y-1/2"/>
            <input type="text" placeholder="Search video ID, title, or resource URL..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-9 pr-4 py-2 bg-stone-950 border border-stone-800 rounded-lg text-sm text-white focus:outline-none focus:border-emerald-500 font-mono text-xs"/>
          </div>
          <button type="submit" className="px-4 py-2 bg-emerald-500 hover:bg-emerald-400 text-black font-bold rounded-lg text-xs transition">
            Search
          </button>
        </form>
      ...
      {/* Error display */}
      {error && (<div className="p-4 bg-red-500/10 border border-red-500/20 text-red-400 rounded-lg text-sm">
          {error}
        </div>)}
      ...
      <div className="p-6 rounded-xl bg-stone-900 border border-stone-800 overflow-hidden">
        {loading ? (<div className="flex items-center justify-center py-16">
          ...
          </div>) : logs.length === 0 ? (<div className="text-center py-16 text-stone-500 text-sm">
            ...
            <table className="w-full text-left text-sm text-stone-300">
              ...
              <tbody className="divide-y divide-stone-800/40">
                {logs.map((log) => (<tr key={log.id} className="hover:bg-stone-800/20 transition">
                    ...
                    <td className="py-4 max-w-xs">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        <a href={`https://youtu.be/${log.vid}`} target="_blank" rel="noreferrer" className="text-stone-500 hover:text-stone-300" title="View on YouTube">
                    ...
                    <td className="py-4 max-w-md">
                      <div className="flex items-center gap-2 bg-stone-950 p-2 rounded-lg border border-stone-800">
                        ...
                        <button onClick={() => handleCopyLink(log.playUrl, log.id)} className="p-1 text-stone-400 hover:text-white transition shrink-0" title="Copy Resource URL">
                          {copiedId === log.id ? (<lucide_react_1.Check className="w-3.5 h-3.5 text-emerald-400"/>) : (<lucide_react_1.Copy className="w-3.5 h-3.5"/>)}
                        </button>
                        <a href={log.playUrl} target="_blank" rel="noreferrer" className="p-1 text-emerald-400 hover:text-emerald-300 transition shrink-0" title="Test Stream / Download URL">
                    ...
                    <td className="py-4 text-xs font-mono text-stone-400">
                      <div className="flex items-center gap-1.5 text-stone-300">
                        <lucide_react_1.Globe className="w-3 h-3 text-stone-500"/>
                        {log.ipAddress}
                      </div>
                      {log.userId && (<div className="flex items-center gap-1 text-[10px] text-stone-500 mt-1">
                    ...
                    <td className="py-4 pr-2 text-right">
                      <a href={log.playUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 px-3 py-1.5 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 rounded-md text-xs font-medium transition">
                        <lucide_react_1.PlayCircle className="w-3.5 h-3.5"/>
                        Play
                      </a>
                    </td>
                  </tr>))}
        ...
        {/* Pagination Controls */}
        {totalPages > 1 && (<div className="flex items-center justify-between pt-6 border-t border-stone-800 mt-4">
            <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1 || loading} className="flex items-center gap-1 px-3 py-2 bg-stone-800 hover:bg-stone-700 disabled:opacity-40 rounded-lg text-xs text-stone-300 font-semibold transition">
            ...
            <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages || loading} className="flex items-center gap-1 px-3 py-2 bg-stone-800 hover:bg-stone-700 disabled:opacity-40 rounded-lg text-xs text-stone-300 font-semibold transition">
              Next
