...
function Dashboard() {
    ...
    (0, react_1.useEffect)(() => {
        (0, api_client_1.apiRequest)("/dashboard")
            .then((res) => {
            setData(res);
            setLoading(false);
        })
    ...
    if (error) {
        return (<div className="p-6 bg-red-500/10 border border-red-500/20 rounded-xl text-red-400">
        Error: {error}
      </div>);
    }
    const { cards, topCategories, topSearches, recentActivities, charts } = data;
    const statCards = [
        { name: "Total Listeners", value: cards.totalUsers, icon: lucide_react_1.Users, color: "text-blue-400" },
        { name: "Premium Subscribers", value: cards.premiumUsers, icon: lucide_react_1.Award, color: "text-emerald-400" },
        { name: "Daily Active Users", value: cards.dailyActiveUsers, icon: lucide_react_1.Activity, color: "text-purple-400" },
        { name: "Global Plays", value: cards.playCount, icon: lucide_react_1.PlayCircle, color: "text-pink-400" },
        { name: "Favourited Tracks", value: cards.favoriteCount, icon: lucide_react_1.Heart, color: "text-red-400" },
    ];
    return (<div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
        {statCards.map((card, idx) => {
            ...
            return (<div key={idx} className="p-6 rounded-xl bg-stone-900 border border-stone-800 flex flex-col justify-between">
              ...
              <div>
                ...
                <div className="flex items-center gap-1 mt-1 text-[10px] text-emerald-400 font-medium">
                  <lucide_react_1.ArrowUpRight className="w-3.5 h-3.5"/>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="p-6 rounded-xl bg-stone-900 border border-stone-800">
          ...
          <div className="h-64 flex items-end justify-between gap-2 pt-6">
            {charts.userGrowthChart.map((month, idx) => {
            const maxUsers = Math.max(...charts.userGrowthChart.map((m) => m.users));
            const heightPercent = (month.users / maxUsers) * 100;
            return (<div key={idx} className="flex-1 flex flex-col items-center gap-2 group">
                  <div className="w-full flex items-end justify-center gap-1 h-44">
                    {/* Access Users */}
                    <div style={{ height: `${heightPercent}%` }} className="w-1/2 bg-emerald-500/20 group-hover:bg-emerald-500/40 rounded-t-sm transition-all duration-300 relative">
                    ...
                    {/* Premium Users */}
                    <div style={{ height: `${(month.premium / maxUsers) * 100}%` }} className="w-1/2 bg-emerald-500 group-hover:bg-emerald-400 rounded-t-sm transition-all duration-300 relative">
        ...
        <div className="p-6 rounded-xl bg-stone-900 border border-stone-800">
          ...
          <div className="h-64 flex items-end justify-between gap-2 pt-6">
            {charts.playsChart.map((day, idx) => {
            const maxPlays = Math.max(...charts.playsChart.map((d) => d.plays));
            ...
            return (<div key={idx} className="flex-1 flex flex-col items-center gap-2 group">
                  <div className="w-8 bg-stone-800 group-hover:bg-emerald-500/20 border border-stone-700/50 hover:border-emerald-500/30 rounded-t-md h-44 flex items-end justify-center overflow-hidden transition-all duration-200">
                    <div style={{ height: `${heightPercent}%` }} className="w-full bg-emerald-500/70 group-hover:bg-emerald-500 rounded-t-sm transition-all duration-500 relative">
        ...
        <div className="p-6 rounded-xl bg-stone-900 border border-stone-800 lg:col-span-1 space-y-6">
          <div>
            <h3 className="text-sm font-semibold text-stone-300 uppercase tracking-wider flex items-center gap-2 mb-4">
              <lucide_react_1.Search className="w-4 h-4 text-emerald-400"/>
              Trending Search Terms
            </h3>
            <div className="space-y-2">
              {topSearches.map((item, idx) => (<div key={idx} className="flex items-center justify-between p-3 rounded-lg bg-stone-950 border border-stone-900/50">
          ...
          <div>
            <h3 className="text-sm font-semibold text-stone-300 uppercase tracking-wider flex items-center gap-2 mb-4">
              <lucide_react_1.Flame className="w-4 h-4 text-orange-400"/>
              Top Categories
            </h3>
            <div className="space-y-2">
              {topCategories.map((item, idx) => (<div key={idx} className="flex items-center justify-between p-3 rounded-lg bg-stone-950 border border-stone-900/50">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm text-stone-300">
              ...
              <tbody className="divide-y divide-stone-800/40">
                {recentActivities.map((act) => (<tr key={act.id} className="hover:bg-stone-800/20">
